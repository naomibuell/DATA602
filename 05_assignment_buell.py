...
def exercise12(n):
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    checkerboard_matrix = np.ones([n*2, n*2])
    evens = np.arange(n)*2
    odds = evens + 1
    checkerboard_matrix[odds] = 0  # Set even rows' to 0
    alternating = checkerboard_matrix[:,0]  # Get an alternating array of 1 x 2n, starting with 1
    reverse_alternating = alternating[::-1] # Reverse the alternation to start with 0 instead
    alternating_column = reverse_alternating.reshape(-1, 1) # Transpose this to be column-wise
    checkerboard_matrix[:, odds] = alternating_column # Revise odd columns to be the reverse alternation of the even columns
    # ------ Place code above here /\ /\ /\ ------ 
...
def exercise13(n):
    ...
    # generate random integers between 0 and n
    random_ints = np.random.randint(n+1, size = n)

    # generate date range for index
    date_range_index = pd.date_range(start=start, periods = n)

    # create pandas Series 
    s = pd.Series(random_ints, index = date_range_index)

    # plot cumulative sum on chart
    cumulative_sum = s.cumsum() # get cumulative sum of the random integers in the series
    cumulative_sum.plot()
    plt.show()
...
def exercise14(words):
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    df = pd.Series(words).map(lambda word: len(word))  # Create a Series and map word lengths
    # ------ Place code above here /\ /\ /\ ------ 
    return df
